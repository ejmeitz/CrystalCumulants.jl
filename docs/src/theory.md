...
1. **sTDEP** — self-consistent 2nd- through 4th-order IFCs via `make_stdep_ifcs`
2. **Thermodynamic properties** — `crystal_thermodynamic_properties` combines \(\mathcal{F}_\mathrm{H}\), sampled \(V_0\), and analytical \(\mathcal{F}_1\), \(\mathcal{F}_2\)
...
See the [Julia](@ref Julia) page for a worked example and the [Home](index.md) page for convergence settings (`nconf`, `free_energy_q_mesh`, `size_study`).
